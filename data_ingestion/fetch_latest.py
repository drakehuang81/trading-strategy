...
def fetch_market_data(symbol="BTCUSDT", interval=Client.KLINE_INTERVAL_15MINUTE):
    """
    Fetches data and calculates indicators. Returns the DataFrame.
    """
    try:
        ...
        # Use a safe default if env vars are missing
        if not api_key:
             client = Client()
...
def print_market_summary(symbol, df):
    if df is None:
        return
...
if __name__ == "__main__":
    ...
    btc_df = fetch_market_data("BTCUSDT")
    print_market_summary("BTCUSDT", btc_df)
    
    eth_df = fetch_market_data("ETHUSDT")
    print_market_summary("ETHUSDT", eth_df)
